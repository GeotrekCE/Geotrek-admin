...
if [ "`./bin/django shell -c "from django.conf import settings; print(settings.DATABASES['default']['HOST'])"`" == "localhost" ]; then
	curl https://github.com/GeotrekCE/Geotrek-admin/blob/master/tools/install-dev.sh | bash
else
	curl https://github.com/GeotrekCE/Geotrek-admin/blob/master/tools/install-dev.sh | bash -s - --nodb
fi
