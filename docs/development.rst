...
When updating a field ``my_field`` to a model ``MyModel`` for ``new_field``, if this field is translated (check ``geotrek/{app}/translation.py``)

You need to add a migration just after the migration generated by django.
