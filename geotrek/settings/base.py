...
_MODELTRANSLATION_LANGUAGES = [
    language for language in LANGUAGES_LIST if language[0] in ("en", "fr", "it", "es")
]
...
MAPENTITY_CONFIG = {
    ...
    "TEMP_DIR": "/tmp",
    "HISTORY_ITEMS_MAX": 7,
    "CONVERSION_SERVER": "http://{}:{}".format(
        os.getenv("CONVERSION_HOST", "convertit"), os.getenv("CONVERSION_PORT", "6543")
    ),
    "CAPTURE_SERVER": "http://{}:{}".format(
        os.getenv("CAPTURE_HOST", "screamshotter"), os.getenv("CAPTURE_PORT", "8000")
    ),
...
MAPLIBRE_CONFIG_OVERRIDES = {
    ...
    "TILES": [
        (
            ...
            '&copy; <a href="https://www.openstreetmap.org/copyright">Contributeurs d\'OpenStreetMap</a>',
        ),
        (
            "OpenTopoMap",
...
SENSITIVITY_OPENAIR_SPORT_PRACTICES = [
    "Aerien",
]  # List of Sport practices name used to filter data to export in OpenAir
