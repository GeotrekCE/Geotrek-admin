...
class TestModelLand(TestCase):
    ...
    def test_circulationtype_value_default_structure(self):
        structure = Structure.objects.create(name="Structure_1")
        pt = CirculationType.objects.create(
            name="CirculationType_1", structure=structure
        )
    ...
    def test_authorizationtype_value_default_structure(self):
        structure = Structure.objects.create(name="Structure_1")
        pt = AuthorizationType.objects.create(
            name="AuthorizationType_1", structure=structure
        )
    ...
    def test_circulationedge_display(self):
        ...
        self.assertEqual(
            circulation_edge.name_display,
            f'<a data-pk="{circulation_edge.pk}" href="/circulationedge/{circulation_edge.pk}/" >{circulation_edge.circulation_type}</a>',
        )
