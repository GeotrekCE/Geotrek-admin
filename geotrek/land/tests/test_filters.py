...
class LandFiltersTest(TestCase):
    ...
    def _filter_by_edge(self, edgefactoryclass, key, getvalue):
        ...
        useless_edge, seek_edge = self.create_pair_of_distinct_topologies(
            edgefactoryclass, useless_path, seek_path
        )
    ...
    def test_filter_by_physical_edge(self):
        self._filter_by_edge(
            PhysicalEdgeFactory, "physical_type", lambda edge: edge.physical_type.pk
        )

    def test_filter_by_land_edge(self):
        self._filter_by_edge(
            LandEdgeFactory, "land_type", lambda edge: edge.land_type.pk
        )
    ...
    def test_filter_by_circulation_type(self):
        self._filter_by_edge(
            CirculationEdgeFactory,
            "circulation_type",
            lambda edge: edge.circulation_type.pk,
        )
    ...
    def test_filter_by_authorization_type(self):
        self._filter_by_edge(
            CirculationEdgeFactory,
            "authorization_type",
            lambda edge: edge.authorization_type.pk,
        )

    def test_filter_by_competence_edge(self):
        self._filter_by_edge(
            CompetenceEdgeFactory, "competence", lambda edge: edge.organization.pk
        )

    def test_filter_by_work_management_edge(self):
        self._filter_by_edge(
            WorkManagementEdgeFactory, "work", lambda edge: edge.organization.pk
        )

    def test_filter_by_signage_management_edge(self):
        self._filter_by_edge(
            SignageManagementEdgeFactory, "signage", lambda edge: edge.organization.pk
        )
