...
class TopologyFilterCompetenceEdge(TopologyFilter):
    ...
    def values_to_edges(self, values):
        return CompetenceEdge.objects.filter(organization__in=values).select_related(
            "organization"
        )
...
class TopologyFilterSignageManagementEdge(TopologyFilter):
    ...
    def values_to_edges(self, values):
        return SignageManagementEdge.objects.filter(
            organization__in=values
...
class TopologyFilterWorkManagementEdge(TopologyFilter):
    ...
    def values_to_edges(self, values):
        return WorkManagementEdge.objects.filter(
            organization__in=values
        ).select_related("organization")
