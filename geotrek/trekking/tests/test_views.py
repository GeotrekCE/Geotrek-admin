...
from .factories import (
    ...
    TrekFactory,
    TrekNetworkFactory,
    TrekWithInfrastructuresFactory,
    TrekWithPOIsFactory,
...
class POIViewsTest(CommonTest):
    ...
    def test_list_in_csv(self):
        ...
        for row in reader:
            self.assertSetEqual(
                {elem for elem in row["Districts"].split(", ")},
                {"Trifouilli", "Refouilli"},
            )
...
class TrekCustomViewTests(TrekkingManagerTest):
    ...
    def test_trek_infrastructure_geojson(self):
        ...
        names = [
            ...
            for feature in infrastructureslayer["features"]
        ]
...
class TrekCustomPublicViewTests(TrekkingManagerTest):
    ...
    def test_overriden_public_template(self, exists_patched):
        overriden_template = os.path.join(
            settings.VAR_DIR, "conf", "extra_templates", "trekking", "trek_public.odt"
        )
...
class TrekPointsReferenceTest(TrekkingManagerTest):
    ...
    def setUpTestData(cls):
        ...
        cls.trek.points_reference = MultiPoint(
            [Point(0, 0), Point(1, 1)], srid=settings.SRID
        )
        cls.trek.save()
...
class TrekViewTranslationTest(TrekkingManagerTest):
    ...
    def test_geojson_translation(self):
        for lang, expected in [("fr", self.trek.name_fr), ("it", self.trek.name_it)]:
            self.login()
            response = self.client.get(
                reverse("trekking:trek-drf-list", format="geojson"),
    ...
    def test_poi_geojson_translation(self):
        ...
        poi.save()
        trek = TrekFactory.create(
            paths=[(p1, 0.5, 1)], published_fr=True, published_it=True
        )
