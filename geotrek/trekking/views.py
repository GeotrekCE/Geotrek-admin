...
class TrekDocumentPublicMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        if settings.TREK_EXPORT_INFRASTRUCTURE_LIST_LIMIT > 0:
            infrastructures = infrastructures[:settings.TREK_EXPORT_INFRASTRUCTURE_LIST_LIMIT]
            signages = signages[:settings.TREK_EXPORT_INFRASTRUCTURE_LIST_LIMIT]
        letters_i = alphabet_enumeration(len(infrastructures))
        letters_s = alphabet_enumeration(len(signages))
        for i, infrastructure in enumerate(infrastructures):
            infrastructure.letter = letters_i[i]
        for j, signage in enumerate(signages):
            signage.letter = letters_s[j]
