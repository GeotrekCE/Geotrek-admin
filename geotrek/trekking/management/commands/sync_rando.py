...
class Command(BaseCommand):
    ...
    def sync_geojson(self, lang, viewset, name):
        ...
        name = os.path.join('api', '{name}-{lang}.geojson'.format(name=name, lang=lang))
    ...
    def sync_pois(self, lang):
        ...
        for trek in trekking_models.Trek.objects.filter(**{'published_{lang}'.format(lang=lang): True}):
            name = os.path.join('api', 'treks', str(trek.pk), 'pois-{lang}.geojson'.format(lang=lang))
    ...
    def sync_exports(self, lang, model, view, ext):
        for obj in model.objects.filter(**{'published_{lang}'.format(lang=lang): True}):
            ...
            name = os.path.join('api', '{modelname}s'.format(modelname=modelname), str(obj.pk), '{slug}-{lang}.{ext}'.format(slug=obj.slug, lang=lang, ext=ext))
