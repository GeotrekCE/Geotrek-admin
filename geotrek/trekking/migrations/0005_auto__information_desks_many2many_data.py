...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding M2M table for field information_desks on 'Trek'
        m2m_table_name = 'o_r_itineraire_renseignement'
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('trek', models.ForeignKey(orm[u'trekking.trek'], null=False)),
            ('informationdesk', models.ForeignKey(orm[u'trekking.informationdesk'], null=False))
        ))
        db.create_unique(m2m_table_name, ['trek_id', 'informationdesk_id'])

        # Migrate data of FK to M2M
        for trek in orm.Trek.objects.all():
            if trek.information_desk:
                trek.information_desks.add(trek.information_desk)
                trek.save()
