...
@override_settings(
    SURICATE_REPORT_SETTINGS=SURICATE_REPORT_SETTINGS,
    SURICATE_MANAGEMENT_SETTINGS=SURICATE_MANAGEMENT_SETTINGS,
)
class SuricateTests(TestCase):
    """Test Suricate API"""

    def build_get_request_patch(
        self, mocked: MagicMock, cause_JPG_error=False, remove_one_alert=False
    ):
    ...
    def setUpTestData(cls):
        ...
        cls.programmed_status = ReportStatusFactory(
            identifier="programmed", label="Programmé", timer_days=7
        )
...
class SuricateAPITests(SuricateTests):
    ...
    def test_get_alerts_creates_alerts_and_send_mail(self, mocked_get, mocked_logger):
        ...
        self.assertEqual(
            r.comment,
            "Ne pas prendre la route Départementale 155 en direction de Malons",
        )
    ...
    @mock.patch("geotrek.feedback.parsers.logger")
    @mock.patch("geotrek.feedback.helpers.requests.get")
    def test_get_alerts_creates_alerts_and_sends_no_mail(
        self, mocked_get, mocked_logger
    ):
    ...
    @mock.patch("geotrek.feedback.parsers.logger")
    @mock.patch("geotrek.feedback.helpers.requests.get")
    def test_get_alerts_does_not_override_internal_status(
        self, mocked_get, mocked_logger
    ):
    ...
    @mock.patch("geotrek.feedback.parsers.logger")
    @mock.patch("geotrek.feedback.helpers.requests.get")
    def test_sync_handles_malformed_images(
        self, mocked_get, mocked_logger, mocked_save
    ):
        self.build_get_request_patch(mocked_get)
    ...
    @mock.patch("geotrek.feedback.parsers.logger")
    @mock.patch("geotrek.feedback.helpers.requests.get")
    def test_failed_attachments_are_downloaded_on_next_sync(
        self, mocked_get, mocked_logger
    ):
        ...
        for atta in Attachment.objects.all():
            ...
            self.assertTrue(
                atta.attachment_file.storage.exists(atta.attachment_file.name),
                atta.attachment_file.name,
            )
    ...
    @mock.patch("geotrek.feedback.helpers.requests.get")
    def test_save_on_report_doesnt_post_to_suricate_in_no_suricate_mode(
        self, post_report
    ):
    ...
    def test_connection_test(self, mock_get, mocked_stdout):
        ...
        # Assert outputs OK
        self.assertEqual(
            mocked_stdout.getvalue(), "API Standard :\nOK\nAPI Gestion :\nOK\n"
        )
    ...
    def test_relocated_report_is_deleted_on_next_sync(self, mocked_get, mocked_logger):
        ...
        self.assertEqual(
            Report.objects.filter(
                external_uuid="742CBF16-5056-AA2B-DD1FD403F72D6B9B"
            ).count(),
            1,
        )
        self.assertEqual(Report.objects.count(), 8)
...
class SuricateWorkflowTests(SuricateTests):
    ...
    def setUpTestData(cls):
        ...
        cls.classified_status = ReportStatusFactory(
            identifier="classified", label="Classé sans suite"
        )
        cls.waiting_status = ReportStatusFactory(
            identifier="waiting", label="En cours", timer_days=6
        )
        cls.rejected_status = ReportStatusFactory(identifier="rejected", label="Rejeté")
        cls.late_intervention_status = ReportStatusFactory(
            identifier="late_intervention", label="Intervention en retard"
        )
        cls.late_resolution_status = ReportStatusFactory(
            identifier="late_resolution", label="Resolution en retard"
        )
        cls.solved_intervention_status = ReportStatusFactory(
            identifier="solved_intervention", label="Intervention terminée"
        )
    ...
    def inner(self, *args, **kwargs):
        ...
        try:
            with override_settings(
                SURICATE_REPORT_ENABLED=False, SURICATE_WORKFLOW_ENABLED=False
            ):
        ...
        try:
            with override_settings(
                SURICATE_REPORT_ENABLED=True, SURICATE_WORKFLOW_ENABLED=False
            ):
                test_func(self, *args, **kwargs)
        except AssertionError as e:
...
class TestWorkflowFirstSteps(SuricateWorkflowTests):
    ...
    def test_classify_alert_notifies_suricate_when_workflow_enabled(
        self,
        mocked_notify_suricate_status,
        mocked_mail_sentinel,
        mocked_post,
        mocked_get,
    ):
        form = ReportForm(
        ...
        form.save()
        mocked_mail_sentinel.assert_called_once_with(
            self.report_filed_1.formatted_external_uuid, "Problème déjà réglé"
        )
        mocked_notify_suricate_status.assert_called_once_with(
            self.report_filed_1.formatted_external_uuid,
            self.classified_status.identifier,
            "Problème déjà réglé",
            "Je ne fais rien",
        )
    ...
    @mock.patch("geotrek.feedback.helpers.SuricateMessenger.message_sentinel")
    @mock.patch("geotrek.feedback.helpers.SuricateMessenger.update_status")
    def test_classify_alert_does_not_notify_suricate_when_workflow_disabled(
        self,
        mocked_notify_suricate_status,
        mocked_mail_sentinel,
        mocked_post,
