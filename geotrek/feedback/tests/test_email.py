...
class EmailSendingTest(SuricateTests):
    ...
    def test_email_failed_warns_and_is_stored_as_pending(self, mocked):
        self.assertRaises(
            Exception, SuricateParser().send_workflow_manager_new_reports_email(set())
        )
    ...
    def test_email_format_and_content(self):
        ReportFactory.create(
            email="john.doe@nowhere.com", comment="This is a 'comment'"
        )
...
class TestPendingEmail(SuricateTests):
    ...
    def test_email_failing_for_manager(self, mocked):
        ...
        with override_settings(
            ...
            self.assertRaises(
                Exception,
                SuricateParser().send_workflow_manager_new_reports_email(set()),
            )
            mocked.error.assert_called_with(
                "Email could not be sent to Workflow Managers."
            )
            self.assertEqual(PendingEmail.objects.count(), 1)
            pending_mail = PendingEmail.objects.first()
            self.assertEqual(
        ...
        with override_settings(
            EMAIL_BACKEND="geotrek.feedback.tests.test_email.FailingEmailBackend2"
        ):
