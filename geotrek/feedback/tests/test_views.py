...
from .factories import ReportActivityFactory, ReportCategoryFactory, ReportProblemMagnitudeFactory, ReportStatusFactory, \
    ReportFactory
from ..models import ReportActivity, ReportCategory, ReportProblemMagnitude, ReportStatus
...
class ReportViewsetMailSend(TestCase):
    ...
    def test_mail_send_on_request(self, mocked_post):
        ...
        created_report = feedback_models.Report.objects.filter(
            email="test_post@geotrek.local"
        ).first()
...
class ReportSerializationOptimizeTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.classified_status = feedback_factories.ReportStatusFactory(
            identifier="classified", label="Classé sans suite"
        )
        cls.filed_status = feedback_factories.ReportStatusFactory(
            identifier="filed", label="Classé sans suite"
        )
        cls.classified_report_1 = feedback_factories.ReportFactory(
            status=cls.classified_status
        )
        cls.classified_report_2 = feedback_factories.ReportFactory(
            status=cls.classified_status
        )
        cls.classified_report_3 = feedback_factories.ReportFactory(
            status=cls.classified_status
        )
    ...
    def test_report_layer_cache(self):
        ...
        self.classified_report_4 = feedback_factories.ReportFactory(
            status=self.classified_status
        )
...
class ReportViewsTest(CommonTest):
    ...
    def test_crud_status(self):
        ...
        response = self.client.get(
            obj.get_detail_url().replace(str(obj.pk), "1234567890")
        )
    ...
    def test_custom_columns_mixin_on_export(self):
        ...
        with override_settings(
            COLUMNS_LISTS={"feedback_export": self.extra_column_list}
        ):
            self.assertEqual(
                import_string(
    ...
    def test_coordinates_in_export(self):
        ...
        self.assertAlmostEqual(float(dict_from_csv["Coord_X"]), 4.62, 1)
        self.assertAlmostEqual(float(dict_from_csv["Coord_Y"]), 43.88, 1)

    @test_for_all_suricate_modes
    def test_cities_in_export(self):
        "Check cities in CSV exports"

        self.modelfactory(geom=Point(5, 5, srid=2154))
        city, created = City.objects.get_or_create(
            code="30100",
            name="Alès",
            geom="SRID=2154;MULTIPOLYGON(((0 0, 0 10, 10 10, 10 0, 0 0)))",
        )
        response = self.client.get("/report/list/export/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get("Content-Type"), "text/csv")
...
class SuricateViewPermissions(AuthentFixturesMixin, TestCase):
    ...
    def test_can_delete_intervention(self):
        self.client.force_login(user=self.admin)
        response = self.client.get(
            f"/intervention/edit/{self.unrelated_intervention.pk}/", follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("delete", response.content.decode("utf-8"))
        self.assertNotIn("disabled delete", response.content.decode("utf-8"))
...
class ReportReferencesTest(TestCase):
    ...
    def test_data(self):
        ...
        self.assertEqual(len(data["reportactivity"]), ReportActivity.objects.all().count())
        self.assertEqual(len(data["reportcategory"]), ReportCategory.objects.all().count())
        self.assertEqual(len(data["reportproblemmagnitude"]), ReportProblemMagnitude.objects.all().count())
        self.assertEqual(len(data["reportstatus"]), ReportStatus.objects.all().count())
        self.assertEqual(data["reportactivity"][0], {"id": self.activity.id, "name": self.activity.label})
        self.assertEqual(data["reportcategory"][0], {"id": self.category.id, "name": self.category.label})
        self.assertEqual(data["reportproblemmagnitude"][0], {"id": self.problem_magnitude.id, "name": self.problem_magnitude.label})
        self.assertEqual(data["reportstatus"][0], {"id": self.status.id, "name": self.status.label})
        self.assertEqual(data["pictogram"], {'url': 'http://testserver/static/images/report.png'})
...
class ReportGTAMTest(TestCase):
    ...
    def test_data(self):
        token = self.authenticate(self.user)
        list_url = f"/api/report/drf/reports?format=gtam"
        ...
        reports = [
            {
                'id': self.report.id,
                'date_insert': self.report.date_insert.isoformat().replace('+00:00', 'Z'),
                'date_update': self.report.date_update.isoformat().replace('+00:00', 'Z'),
