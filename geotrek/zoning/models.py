...
if settings.TREKKING_TOPOLOGY_ENABLED:
    ...
    Topology.add_property('areas', lambda self: uniquify(intersecting(RestrictedArea, self))
                                       if self.topo_object.ispoint() else uniquify(map(attrgetter('restricted_area'),
                                                                                       self.area_edges)),
                          _(u"Restricted areas"))
...
if settings.TREKKING_TOPOLOGY_ENABLED:
    ...
    Topology.add_property('cities', lambda self: uniquify(intersecting(City, self,))
                                        if self.topo_object.ispoint() else uniquify(map(attrgetter('city'),
                                                                                        self.city_edges)),
                          _(u"Cities"))
...
if settings.TREKKING_TOPOLOGY_ENABLED:
    ...
    Topology.add_property('districts', lambda self: uniquify(intersecting(District, self))
                                           if self.topo_object.ispoint() else uniquify(map(attrgetter('district'),
                                                                                           self.district_edges)),
                          _(u"Districts"))
