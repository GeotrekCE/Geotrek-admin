...
if settings.TREKKING_TOPOLOGY_ENABLED:
    ...
    Topology.add_property('sorted_cities',
                          lambda self: uniquify(intersecting(City, self, distance=0)), _(u"Sorted cities"))
    Topology.add_property('cities',
                          lambda self: uniquify(intersecting(City, self, distance=0, ordering=False)), _(u"Cities"))
    Intervention.add_property('city_edges', lambda self: self.topology.city_edges if self.topology else [],
                              _(u"City edges"))
    Intervention.add_property('cities', lambda self: self.topology.sorted_cities if self.topology else [], _(u"Cities"))
    ...
    Topology.add_property('sorted_cities', lambda self: uniquify(intersecting(City, self, distance=0)), _(u"Sorted cities"))
    Topology.add_property('cities',
                          lambda self: uniquify(intersecting(City, self, distance=0, ordering=False)), _(u"Cities"))
