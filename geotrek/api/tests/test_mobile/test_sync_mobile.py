...
class SyncMobileTilesTest(VarTmpTestCase):
    ...
    def test_tiles(self, mock_tiles, mock_tileslist):
        output = StringIO()
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000', verbosity=2, stdout=output)
        zfile = zipfile.ZipFile(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', 'global.zip'))
    ...
    def test_tiles_with_treks(self, mock_tiles, mock_prepare, mock_tileslist):
        ...
        zfile_global = zipfile.ZipFile(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', 'global.zip'))
        ...
        zfile_trek = zipfile.ZipFile(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', '{}.zip'.format(trek.pk)))
        ...
        self.assertFalse(os.path.exists(os.path.join(os.path.join('var', 'tmp', 'tmp_sync'), 'nolang', '{}.zip'.format(trek_not_same_portal.pk))))
        self.assertTrue(os.path.exists(os.path.join(os.path.join('var', 'tmp', 'tmp_sync'), 'nolang', '{}.zip'.format(trek_multi.pk))))
...
class SyncMobileFailTest(VarTmpTestCase):
    ...
    def test_fail_directory_not_empty(self):
        os.makedirs(os.path.join('var', 'tmp', 'tmp_sync', 'other'))
        with self.assertRaisesRegex(CommandError, "Destination directory contains extra data"):
            management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
    ...
    def test_fail_url_ftp(self):
        with self.assertRaisesRegex(CommandError, "url parameter should start with http:// or https://"):
            management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='ftp://localhost:8000',
                                    skip_tiles=True, verbosity=2)
    ...
    def test_attachments_missing_from_disk(self, mocke):
        ...
        self.assertFalse(os.path.exists(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', 'media', 'trekking_trek')))
    ...
    def test_response_view_exception(self, mocke):
        ...
        with self.assertRaisesRegex(CommandError, 'Some errors raised during synchronization.'):
            management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000', portal='portal',
...
class SyncMobileSpecificOptionsTest(TranslationResetMixin, VarTmpTestCase):
    ...
    def test_lang(self):
        ...
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'fr', 'flatpages.json'), 'r') as f:
    ...
    def test_sync_https(self):
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='https://localhost:8000',
...
class SyncMobileFlatpageTest(TranslationResetMixin, VarTmpTestCase):
    ...
    def test_sync_flatpage(self):
        ...
        for lang in settings.MODELTRANSLATION_LANGUAGES:
            with open(os.path.join(os.path.join('var', 'tmp', 'tmp_sync'), lang, 'flatpages.json'), 'r') as f:
                flatpages = json.load(f)
    ...
    def test_sync_filtering_portal(self):
        ...
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'fr', 'flatpages.json'), 'r') as f_file:
            flatpages = json.load(f_file)
            self.assertEqual(len(flatpages), 0)
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'en', 'flatpages.json'), 'r') as f_file:
            flatpages = json.load(f_file)
...
class SyncMobileSettingsTest(TranslationResetMixin, VarTmpTestCase):
    ...
    def test_sync_settings(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        for lang in settings.MODELTRANSLATION_LANGUAGES:
            with open(os.path.join(os.path.join('var', 'tmp', 'tmp_sync'), lang, 'settings.json'), 'r') as f:
    ...
    def test_sync_settings_with_picto_svg(self):
        ...
        image_practice = Image.open(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', pictogram_png[1:]))
        image_desk = Image.open(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', pictogram_desk_png[1:]))
...
class SyncMobileTreksTest(TranslationResetMixin, VarTmpTestCase):
    ...
    def test_sync_treks(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        for lang in settings.MODELTRANSLATION_LANGUAGES:
            with open(os.path.join(os.path.join('var', 'tmp', 'tmp_sync'), lang, 'treks.geojson'), 'r') as f:
    ...
    def test_sync_treks_by_pk(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'en', str(self.trek_1.pk), 'trek.geojson'), 'r') as f:
            trek_geojson = json.load(f)
    ...
    def test_sync_treks_with_portal(self):
        ...
        self.assertFalse(os.path.exists(
            os.path.join('var', 'tmp', 'tmp_sync', 'en', str(self.trek_3.pk), 'trek.geojson')
        ))
        for lang in settings.MODELTRANSLATION_LANGUAGES:
            with open(os.path.join('var', 'tmp', 'tmp_sync', lang, 'treks.geojson'), 'r') as f:
                trek_geojson = json.load(f)
                self.assertEqual(len(trek_geojson['features']),
                                 Trek.objects.filter(**{'published_{}'.format(lang): True})
                                 .filter(Q(portal__name__in=(self.portal_a,)) | Q(portal=None)).count())
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'en', str(self.trek_1.pk), 'touristic_contents.geojson'), 'r') as f:
        ...
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'en', str(self.trek_1.pk), 'touristic_events.geojson'), 'r') as f:
    ...
    def test_sync_pois_by_treks(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'en', str(self.trek_1.pk), 'pois.geojson'), 'r') as f:
            trek_geojson = json.load(f)
    ...
    def test_medias_treks(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', str(self.trek_1.pk),
        ...
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', str(self.trek_1.pk), 'media',
                                                    'upload')))
    ...
    def test_medias_treks_multiple_picture(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        self.assertEqual(2, len(os.listdir(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', str(self.trek_1.pk),
                                                        'media', 'paperclip', 'trekking_trek', str(self.trek_1.pk)))))
        self.assertEqual(2, len(os.listdir(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', str(self.trek_1.pk),
                                                        'media', 'paperclip', 'trekking_poi', str(self.poi_1.pk)))))
        self.assertEqual(1, len(os.listdir(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', str(self.trek_1.pk),
                                                        'media', 'paperclip', 'tourism_touristiccontent',
    ...
    def test_streaminghttpresponse(self, mocke):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=2, stdout=output)
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'tmp_sync', 'en', 'treks.geojson')))
    ...
    def test_indent(self):
        ...
        with open(os.path.join('var', 'tmp', 'tmp_sync', 'en', 'treks.geojson')) as f:
    ...
    def test_object_without_pictogram(self):
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=0)
        self.assertIn(pictogram_name_before, os.listdir(os.path.join('var', 'tmp', 'tmp_sync', 'nolang', 'media',
                                                                     'upload')))
        ...
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000',
                                skip_tiles=True, verbosity=0)
        self.assertNotIn(pictogram_name_before, os.listdir(os.path.join('var', 'tmp', 'tmp_sync', 'nolang',
                                                                        'media', 'upload')))

    @skipIf(settings.TREKKING_TOPOLOGY_ENABLED, 'Test without dynamic segmentation only')
    def test_multilinestring(self):
        TrekFactory.create(geom=MultiLineString(LineString((0, 0), (0, 1)), LineString((100, 100), (100, 101))))
        management.call_command('sync_mobile', os.path.join('var', 'tmp', 'tmp_sync'), url='http://localhost:8000', skip_tiles=True,
                                verbosity=0)
        for lang in settings.MODELTRANSLATION_LANGUAGES:
            with open(os.path.join(os.path.join('var', 'tmp', 'tmp_sync'), lang, 'treks.geojson'), 'r') as f:
                trek_geojson = json.load(f)
