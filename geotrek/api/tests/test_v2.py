...
GEOJSON_COLLECTION_STRUCTURE = sorted(["type", "features"])
...
class APIAccessAnonymousTestCase(BaseApiTest):
    ...
    def test_no_pagination(self):
        import pprint
        response = self.get_trek_list({"no_page": "true"})
        self.assertEqual(response.status_code, 200)
        json_response = response.json()
        ...
        self.assertEqual(
            len(json_response[0].get("geometry").get("coordinates")[0]),
            3,
        )

    def test_geojson_no_pagination(self):
        response = self.get_trek_list({"no_page": "true", "format": "geojson"})
        self.assertEqual(response.status_code, 200)
        json_response = response.json()
        print(json_response)
        self.assertIsInstance(json_response, dict)
        self.assertEqual(
            sorted(json_response.keys()), GEOJSON_COLLECTION_STRUCTURE
        )
        self.assertEqual(
    ...
    def test_trek_district_filter(self):
        ...
        dist3 = zoning_factory.DistrictFactory(
            geom="SRID=2154;MULTIPOLYGON(((-10 -10, -10 -9, -9 -9, "
            "-9 -10, -10 -10)))"
        )
        if settings.TREKKING_TOPOLOGY_ENABLED:
            trek_factory.TrekFactory.create(paths=[(path, 0, 1)])
    ...
    def test_hdviewpoint_detail_content(self):
        ...
        self.assertEquals(
            json_response.get("trek").get("uuid"), str(self.treks[0].uuid)
        )
...
class TouristicEventTestCase(BaseApiTest):
    ...
    def test_touristic_event_list_2(self):
        ...
        self.assertLessEqual(
            response.json().get("results")[0]["end_date"],
