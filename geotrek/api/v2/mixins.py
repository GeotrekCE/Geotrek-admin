...
class PDFSerializerMixin:
    def _get_pdf_url_lang(self, obj, lang, portal=None):
        ...
        if settings.ONLY_EXTERNAL_PUBLIC_PDF:
            ...
            if not common_models.Attachment.objects.attachments_for_object_only_type(
                ...
                return None
        urlname = "{}:{}_{}printable".format(
            namespace, modelname, "booklet_" if settings.USE_BOOKLET_PDF else ""
        )
        url = reverse(urlname, kwargs={"lang": lang, "pk": obj.pk, "slug": obj.slug})
        request = self.context.get("request")
        if request:
            url = (
                f"{request.build_absolute_uri(url)}?portal={portal.split(',')[0]}"
                if portal
    ...
    def get_pdf_url(self, obj):
        lang = (
            self.context.get("request").GET.get("language", "all")
            if self.context.get("request")
            else "all"
        )
        portal = (
            self.context.get("request").GET.get("portals", None)
            if self.context.get("request")
            else None
        )
        if lang != "all":
...
class PublishedRelatedObjectsSerializerMixin:
    def get_values_on_published_related_objects(self, related_queryset, field):
        ...
        if language:
            ...
            return list(
                related_queryset.filter(**{published_by_lang: True}).values_list(
                    field, flat=True
                )
            )
        else:
    ...
    def get_value_on_published_related_object(self, related_object, field):
        ...
        if related_object:
            if language:
                if getattr(
                    related_object, build_localized_fieldname("published", language)
                ):
