...
class MenuItemTreeView(GenericAPIView):
    ...
    queryset = (
        ...
        .filter(depth=1)
        .filter(platform__in=[MenuItem.PLATFORM_CHOICES.ALL, MenuItem.PLATFORM_CHOICES.WEB])
    )
    filter_backends = (
        GeotrekPublishedFilter,
    ...
    def _recursive_node_to_dict(self, node):
        ...
        children_qs = self.filter_queryset(
            node.get_children().filter(platform__in=[MenuItem.PLATFORM_CHOICES.ALL, MenuItem.PLATFORM_CHOICES.WEB])
        )
