...
class LabelAccessibilityViewSet(api_viewsets.GeotrekViewSet):
    ...
    queryset = tourism_models.LabelAccessibility.objects.order_by(
        "pk"
...
class TouristicContentCategoryViewSet(api_viewsets.GeotrekViewSet):
    ...
    queryset = tourism_models.TouristicContentCategory.objects.prefetch_related(
        "types"
    ...
    def retrieve(self, request, pk=None, format=None):
        ...
        serializer = api_serializers.TouristicContentCategorySerializer(
...
class InformationDeskViewSet(api_viewsets.GeotrekViewSet):
    ...
    def get_queryset(self):
        with translation.override(self.request.GET.get("language"), deactivate=True):
            return tourism_models.InformationDesk.objects.select_related(
                "label_accessibility", "type"
            ).order_by("name")
    ...
    def retrieve(self, request, pk=None, format=None):
        ...
        serializer = api_serializers.InformationDeskSerializer(
            elem, many=False, context={"request": request}
        )
        return Response(serializer.data)
...
class TouristicEventTypeViewSet(api_viewsets.GeotrekViewSet):
    ...
    queryset = tourism_models.TouristicEventType.objects.order_by(
        "pk"
    )  # Required for reliable pagination
...
class TouristicEventViewSet(api_viewsets.GeotrekGeometricViewset):
    ...
    def get_queryset(self):
        with translation.override(self.request.GET.get("language"), deactivate=True):
            return (
                tourism_models.TouristicEvent.objects.existing()
                .select_related("type")
                .prefetch_related(
                    "themes",
                    "source",
...
class TouristicEventPlaceViewSet(api_viewsets.GeotrekGeometricViewset):
    ...
    def get_queryset(self):
        return (
            tourism_models.TouristicEventPlace.objects.prefetch_related(
                "touristicevents"
            )
            .annotate(geom_transformed=Transform("geom", settings.API_SRID))
            .order_by("name")
        )
