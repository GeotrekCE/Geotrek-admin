...
class AltimetryMixin(models.Model):
    ...
    length = models.FloatField(
        editable=False, default=0.0, null=True, blank=True, verbose_name=_("3D Length")
    )
    ascent = models.IntegerField(
        editable=False, default=0, null=True, blank=True, verbose_name=_("Ascent")
    )
    descent = models.IntegerField(
        editable=False, default=0, null=True, blank=True, verbose_name=_("Descent")
    )
    ...
    slope = models.FloatField(
        editable=False, null=True, blank=True, default=0.0, verbose_name=_("Slope")
    )
    ...
    def get_formatted_elevation_profile_and_limits(self, **kwargs):
        ...
        data["limits"] = dict(
            zip(["ceil", "floor"], AltimetryHelper.altimetry_limits(elevation_profile))
        )
        return data
    ...
    def prepare_elevation_chart(self, language):
        ...
        if is_file_uptodate(path, self.date_update):
            return False
        cairosvg.svg2png(
            bytestring=bytes(self.get_elevation_profile_svg(language)), write_to=path
        )
        return True
...
class Dem(models.Model):
    id = models.AutoField(
        primary_key=True, db_column="rid"
    )  # rid is id column name used by raster2pgsql
