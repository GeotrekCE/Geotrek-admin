...
class CommandLoadDemTest(TransactionTestCase):
    ...
    def test_success_without_replace(self):
        ...
        self.assertIn(
            "Everything looks fine, we can start loading DEM", output_stdout.getvalue()
        )
        dems = Dem.objects.all().annotate(
            int=RasterValue("rast", Point(x=605600, y=6650000, srid=2154))
        )
        value = dems.first()
    ...
    def test_success_with_replace(self):
        ...
        call_command(
            "loaddem", filename, "--replace", verbosity=2, stdout=output_stdout
        )
        self.assertFalse(
            Dem.objects.filter(pk__in=raster_tiles).exists()
        )  # first imported tiles not exist anymore
        dems = Dem.objects.all().annotate(
            int=RasterValue("rast", Point(x=605600, y=6650000, srid=2154))
        )
        value = dems.first()
    ...
    def test_fail_table_altimetry_dem(self):
        ...
        with self.assertRaisesRegex(
            CommandError, "DEM file exists, use --replace to overwrite"
        ):
            call_command("loaddem", filename, verbosity=0)
    ...
    def test_fail_wrong_format(self):
        ...
        with self.assertRaisesRegex(
            CommandError, "DEM format is not recognized by GDAL."
        ):
            call_command("loaddem", filename, verbosity=0)

    @mock.patch(
        "geotrek.altimetry.management.commands.loaddem.Command.call_command_system"
    )
    def test_fail_raster2pgsql_first(self, sp):
        ...
        filename = os.path.join(os.path.dirname(__file__), "data", "elevation.tif")
        with self.assertRaisesRegex(
            CommandError, "Caught Exception: raster2pgsql failed with exit code 1"
        ):
