...
from .models import Contractor, Intervention, Project
...
class PolygonInterventionFilterMixin:
    ...
    def filter(self, qs, values):
        ...
        for element in qs:
            if element.target:
                if not element.target.geom or element.target.geom.intersects(
                    geom_intersect
                ):
...
class PolygonProjectFilterMixin(PolygonInterventionFilterMixin):
    ...
    def filter(self, qs, values):
        ...
        return qs.filter(
            interventions__in=super()
            .filter(interventions, values)
            .values_list("id", flat=True)
        )


class InterventionIntersectionFilterRestrictedAreaType(
    PolygonInterventionFilterMixin, IntersectionFilterRestrictedAreaType
):
    def get_geom(self, value):
        return value.geom
...
class InterventionIntersectionFilterRestrictedArea(
    PolygonInterventionFilterMixin, IntersectionFilterRestrictedArea
):
    def get_geom(self, value):
        return value.geom


class InterventionIntersectionFilterCity(
    PolygonInterventionFilterMixin, IntersectionFilterCity
):
    def get_geom(self, value):
        return value.geom


class InterventionIntersectionFilterDistrict(
    PolygonInterventionFilterMixin, IntersectionFilterDistrict
):
    def get_geom(self, value):
        return value.geom


class PolygonTopologyFilter(
    BboxInterventionFilterMixin, PolygonInterventionFilterMixin, PolygonFilter
):
    pass
...
class ProjectIntersectionFilterRestrictedAreaType(
    PolygonProjectFilterMixin, RightFilter
):
...
class InterventionFilterSet(
    AltimetryInterventionFilterSet, ZoningFilterSet, StructureRelatedFilterSet
):
    ON_CHOICES = (
    ...
    if "geotrek.outdoor" in settings.INSTALLED_APPS:
        ON_CHOICES += (
            ("course", _("Outdoor Course")),
            ("site", _("Outdoor Site")),
        )

    bbox = PolygonTopologyFilter(lookup_expr="intersects")
    begin_date = CustomDateFromToRangeFilter(
    ...
    end_date = CustomDateFromToRangeFilter(
        widget=OneLineRangeWidget(
            attrs={
                "type": "text",
                "class": "minmax-field",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.fields["on"].choices = sorted(
            self.ON_CHOICES, key=lambda modelname: modelname[1]
        )
