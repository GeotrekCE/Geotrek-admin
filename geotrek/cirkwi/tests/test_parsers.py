...
from geotrek.cirkwi.parsers import CirkwiTouristicContentParser, CirkwiTrekParser
...
from geotrek.trekking.tests.factories import DifficultyLevelFactory, PracticeFactory
...
class CirkwiParserTests(TestCase):
    ...
    def test_create_treks(self, mocked_get):
        ...
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTrekParserFr",
            verbosity=2,
            stdout=output,
        )
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTrekParserEn",
            verbosity=0,
        )
        ...
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTrekParserFrUpdateOnly",
            verbosity=0,
        )
        self.assertEqual(Trek.objects.count(), 2)
    ...
    def test_create_touristic_content_no_type(self, mocked_get):
        ...
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTouristicContentDoNotCreateParserEn",
            verbosity=2,
            stdout=output,
        )
        self.assertIn(
            "Type 1 'Eglise' does not exist for category 'Monuments et Architecture'. Please add it",
            output.getvalue(),
        )
    ...
    def test_create_trek_with_missing_locomotion_and_difficulty(self, mocked_get):
        ...
        mocked_get.side_effect = self.make_dummy_get(
            "circuits_wrong_locomotion_and_difficulty.xml"
        )
        # Test Locomotion does not exist
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTrekParserFrNoCreate",
            verbosity=2,
            stdout=output,
        )
        self.assertIn(
        ...
        self.assertIn(
            "Aucune Pratique ne correspond à la Locomotion Cirkwi 'Aviron' (id: '8'). Merci de l'ajouter,",
            output.getvalue(),
        )
        self.assertIn(
    ...
    def test_create_touristic_content(self, mocked_get):
        ...
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTouristicContentParserFr",
            verbosity=0,
        )
        call_command(
            "import",
            "geotrek.cirkwi.tests.test_parsers.TestCirkwiTouristicContentParserEn",
            verbosity=0,
        )
        self.assertEqual(TouristicContent.objects.count(), 2)
        tc = TouristicContent.objects.get(
            name_fr="Tour de lancienne église Sainte-Gertrude"
        )
        ...
        self.assertEqual(
            tc.practical_info_fr,
            "<strong>Adresse : </strong><br>1 route de Bastogne<br>6970 Tenneville<br><br><strong>Horaire : </strong><br>Ouvert du Lundi au Vendredi de 8h à 19h<br><br><strong>Contact : </strong><br>Téléphone: 01 02 03 04 05<br>",
        )
