...
class CirkwiTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.label_1 = LabelFactory(
            advice_fr="Lorep ipsum 1 fr", advice_en="Lorep ipsum 1 en"
        )
        cls.label_2 = LabelFactory(
            advice_fr="Lorep ipsum 2 fr", advice_en="Lorep ipsum 2 en"
        )
    ...
    def test_export_circuits(self):
        ...
        attrs = {
            ...
            "n": self.trek.description.replace("<p>description ", "").replace(
                "</p>", ""
            ),
            "poi_pk": self.poi.pk,
            "poi_title": self.poi.name,
            "poi_date_update": timestamp(self.poi.date_update),
            "poi_description": self.poi.description.replace("<p>", "").replace(
                "</p>", ""
            ),
        }
        self.assertXMLEqual(
    ...
    def test_export_pois_with_attachments(self):
        attachment = AttachmentFactory.create(
            content_object=self.poi, attachment_file=get_dummy_uploaded_image()
        )
        response = self.client.get("/api/cirkwi/pois.xml")
        self.assertEqual(response.status_code, 200)
        attrs = {
    ...
    def test_export_circuits_with_attachments(self):
        attachment = AttachmentFactory.create(
            content_object=self.trek, attachment_file=get_dummy_uploaded_image()
        )
        self.poi.delete()
        response = self.client.get("/api/cirkwi/circuits.xml")
        self.assertEqual(response.status_code, 200)
    ...
    def test_trek_filter_portals(self):
        ...
        response = self.client.get(
            f"/api/cirkwi/circuits.xml?portals={other_portal.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLEqual(
            response.content.decode(),
        ...
        response = self.client.get(
            f"/api/cirkwi/circuits.xml?portals={other_portal.pk},{portal.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLNotEqual(
            response.content.decode(),
    ...
    def test_trek_filter_structures(self):
        ...
        response = self.client.get(
            f"/api/cirkwi/circuits.xml?structures={structure.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLNotEqual(
            response.content.decode(),
        ...
        response = self.client.get(
            f"/api/cirkwi/circuits.xml?structures={other_structure.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLEqual(
            response.content.decode(),
        ...
        response = self.client.get(
            f"/api/cirkwi/circuits.xml?structures={other_structure.pk},{structure.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLNotEqual(
            response.content.decode(),
    ...
    def test_poi_filter_structures(self):
        ...
        # We found no treks with the other structure's id
        response = self.client.get(
            f"/api/cirkwi/pois.xml?structures={other_structure.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLEqual(
            response.content.decode(),
        ...
        response = self.client.get(
            f"/api/cirkwi/pois.xml?structures={other_structure.pk},{structure.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLNotEqual(
            response.content.decode(),
        ...
        response = self.client.get(
            f"/api/cirkwi/pois.xml?structures={other_structure.pk}&structures={structure.pk}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertXMLNotEqual(
            response.content.decode(),
