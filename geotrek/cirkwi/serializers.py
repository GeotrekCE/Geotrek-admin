        ...
        for poi in pois:
            ...
            if poi.type.cirkwi:
                ...
                self.serialize_field(
                    "categorie", str(poi.type.cirkwi.eid), {"nom": poi.type.cirkwi.name}
                )
...
class CirkwiTrekSerializer(CirkwiPOISerializer):
    ...
    def serialize_tags(self, trek):
        tag_ids = list(
            trek.themes.filter(cirkwi_id__isnull=False).values_list(
                "cirkwi_id", flat=True
            )
        )
        tag_ids += trek.accessibilities.filter(cirkwi_id__isnull=False).values_list(
            "cirkwi_id", flat=True
        )
        ...
        if tag_ids:
            ...
            for tag in CirkwiTag.objects.filter(id__in=tag_ids):
                self.serialize_field(
                    "tag_public", "", {"id": str(tag.eid), "nom": tag.name}
                )
    ...
    def serialize(self, treks):
        ...
        for trek in treks:
            ...
            self.serialize_field(
                "fichier_trace", "", {"url": self.request.build_absolute_uri(kml_url)}
            )
