...
from geotrek.outdoor.forms import CourseForm, SiteForm
from geotrek.outdoor.models import OrderedCourseChild
from geotrek.outdoor.tests.factories import CourseFactory, RatingFactory, SiteFactory
...
class SiteFormTest(TestCase):
    def test_ratings_save(self):
        ...
        self.assertListEqual(
            list(site.ratings.all().values_list("pk", flat=True)), [rating.pk]
        )
...
class CourseFormTest(TestCase):
    ...
    def test_rating_save(self):
        ...
        form.save()
        self.assertListEqual(
            list(self.course.ratings.all().values_list("pk", flat=True)),
            [self.rating.pk],
        )
...
class CourseItinerancyTestCase(TestCase):
    ...
    def test_parent_as_child(self):
        ...
        with self.assertRaisesRegex(
            ValidationError, "Cannot use parent course 2 as a child course."
        ):
            form.clean_children_course()
