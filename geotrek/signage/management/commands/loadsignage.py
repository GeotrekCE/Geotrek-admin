...
from geotrek.signage.models import Sealing, Signage, SignageCondition, SignageType
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store",
            dest="name_field",
            help="Name of the field that will be mapped to the Name field in Geotrek",
        )
        parser.add_argument(
            "--type-field",
            "-t",
            action="store",
            dest="type_field",
            help="Name of the field that will be mapped to the Type field in Geotrek",
        )
        parser.add_argument(
            "--condition-field",
            "-c",
            action="store",
            dest="condition_field",
            help="Name of the field that will be mapped to the Condition field in Geotrek",
        )
        parser.add_argument(
            "--manager-field",
            "-m",
            action="store",
            dest="manager_field",
            help="Name of the field that will be mapped to the Manager field in Geotrek",
        )
        parser.add_argument(
            "--sealing-field",
            action="store",
            dest="sealing_field",
            help="Name of the field that will be mapped to the sealing field in Geotrek",
        )
        parser.add_argument(
            "--structure-field",
            "-s",
            action="store",
            dest="structure_field",
            help="Name of the field that will be mapped to the Structure field in Geotrek",
        )
        parser.add_argument(
            "--description-field",
            "-d",
            ...
            help="Name of the field that will be mapped to the Description field in Geotrek",
        )
        parser.add_argument(
            "--year-field",
            "-y",
            action="store",
            dest="year_field",
            help="Name of the field that will be mapped to the Year field in Geotrek",
        )
        parser.add_argument(
            "--code-field",
            action="store",
            dest="code_field",
            help="Name of the field that will be mapped to the Code field in Geotrek",
        )
        parser.add_argument(
            "--eid-field",
            action="store",
            dest="eid_field",
            help="Name of the field that will be mapped to the External ID in Geotrek",
        )
        parser.add_argument(
            "--type-default",
            action="store",
            dest="type_default",
            help="Default value for Type field",
        )
        parser.add_argument(
            "--name-default",
            action="store",
            dest="name_default",
            help="Default value for Name field",
        )
        parser.add_argument(
            "--condition-default",
            action="store",
            dest="condition_default",
            help="Default value for Condition field",
        )
        parser.add_argument(
            "--manager-default",
            action="store",
            dest="manager_default",
            help="Default value for the Manager field",
        )
        parser.add_argument(
            "--sealing-default",
            action="store",
            dest="sealing_default",
            help="Default value for the Sealing field",
        )
        parser.add_argument(
            "--structure-default",
            action="store",
            dest="structure_default",
            help="Default value for Structure field",
        )
        parser.add_argument(
            "--description-default",
            action="store",
            dest="description_default",
            default="",
        ...
        parser.add_argument(
            ...
            action="store",
            dest="year_default",
            help="Default value for Year field",
        )
        parser.add_argument(
            "--code-default",
            action="store",
            dest="code_default",
            default="",
            help="Default value for Code field",
        )

    def check_fields_available_with_default(
        self, available_fields, name_field, default_field, prefix_argument
    ):
        if (name_field and name_field not in available_fields) or (
            not name_field and not default_field
        ):
            self.stdout.write(
                self.style.ERROR(f"Field '{name_field}' not found in data source.")
            )
            self.stdout.write(
                self.style.ERROR(
    ...
    def check_fields_available_without_default(
        self, available_fields, name_field, prefix_argument
    ):
        if name_field and name_field not in available_fields:
            ...
            self.stdout.write(
                self.style.ERROR(f"Change your --{prefix_argument}-field option")
            )
            return False
        return True
        ...
        try:
            for layer in data_source:
                ...
                if not self.check_fields_available_with_default(
                    ...
                    break
                if not self.check_fields_available_with_default(
                    available_fields, field_name, default_name, "name"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_condition_type, "conditions"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_manager, "manager"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_sealing, "sealing"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_structure_type, "structure"
                ):
                    break
                ...
                if not self.check_fields_available_without_default(
                    available_fields, field_description, "description"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_implantation_year, "year"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_eid, "eid"
                ):
                    break
                if not self.check_fields_available_without_default(
                    available_fields, field_code, "code"
                ):
                    break
                ...
                for feature in layer:
                    ...
                    tmp_signage_type = (
                        ...
                        else default_infrastructure_type
                    )
                    signage_type, created = SignageType.objects.get_or_create(
                        label=tmp_signage_type,
                    ...
                    condition = (
                        ...
                        else default_condition_type
                    )
                    if condition:
                        signage_condition, created = (
                    ...
                    sealing = (
                        feature.get(field_sealing)
                        if field_sealing in available_fields
                        else default_sealing
                    )
                    if sealing:
                        sealing, created = Sealing.objects.get_or_create(
                            label=sealing,
                            structure=structure if use_structure else None,
                        )
                        if created and verbosity:
                    ...
                    manager = (
                        feature.get(field_manager)
                        if field_manager in available_fields
                        else default_manager
                    )
                    if manager:
                        manager, created = Organism.objects.get_or_create(
                            organism=manager,
                            structure=structure if use_structure else None,
                        )
                        if created and verbosity:
                    ...
                    structure = (
                        Structure.objects.get(name=feature.get(field_structure_type))
        ...
        with transaction.atomic():
            ...
            if fields_to_integrate["eid"]:
                ...
                signage, created = Signage.objects.update_or_create(
                    eid=eid, defaults=fields_to_integrate
                )
                if conditions:
