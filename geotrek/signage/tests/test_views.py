...
class SignageTemplatesTest(TestCase):
    ...
    def login(self):
        user = User.objects.create_superuser(
            "test", "test@example.com", password="test"
        )
    ...
    def test_direction_field_on_each_line_on_detail_page_when_direction_on_lines_enabled(
        self,
    ):
...
class BladeViewsTest(CommonTest):
    ...
    def get_expected_popup_content(self):
        return (
            ...
            f"        </p>\n    \n"
            f'    <a id="detail-btn" href="/blade/{self.obj.pk}/" class="btn btn-sm btn-info mt-2">Detail sheet</a>\n'
            f"</div>"
        )
    ...
    def test_basic_format_not_ascii(self):
        ...
        for fmt in ("csv", "shp", "gpx"):
            response = self.client.get(
                self.model.get_format_list_url() + "?format=" + fmt
            )
            self.assertEqual(response.status_code, 200, "")
    ...
    def test_csv_format_with_lines(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content.split(b"\r\n")[0],
            b"ID,City,Signage,Printed elevation,Code,Type,Color,"
    ...
    @override_settings(DIRECTION_ON_LINES_ENABLED=True)
    def test_direction_field_is_hidden_on_blade_list_when_direction_on_lines_enabled(
        self,
    ):
        BladeFactory.create()
    ...
    @override_settings(
        ...
        COLUMNS_LISTS={"blade_view": ["direction", "conditions"]},
    )
    def test_direction_custom_field_is_hidden_on_blade_list_when_direction_on_lines_enabled(
        self,
    ):
        BladeFactory.create()
    ...
    @override_settings(DIRECTION_ON_LINES_ENABLED=True)
    def test_direction_field_visibility_on_blade_csv_format_when_direction_on_lines_enabled(
        self,
    ):
        BladeFactory.create()
...
class BladeTemplatesTest(TestCase):
    ...
    @override_settings(DIRECTION_ON_LINES_ENABLED=True)
    def test_direction_field_visibility_on_detail_page_when_direction_on_lines_enabled(
        self,
    ):
        blade = BladeFactory.create(
            direction=BladeDirectionFactory.create(label="A direction on the blade")
...
class SignageViewsTest(CommonTest):
    ...
    def get_expected_datatables_attrs(self):
        return {
            "code": self.obj.code,
            "conditions": ", ".join(
                list(self.obj.conditions.values_list("label", flat=True))
            ),
            "id": self.obj.pk,
