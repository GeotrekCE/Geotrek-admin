...
class ApidaeParser(AttachmentParserMixin, ApidaeBaseParser):
    ...
    def _filter_comm(self, val, code, multiple=True):
        ...
        if not val:
            return None
        vals = [
            subval["coordonnees"]["fr"]
            for subval in val
            if subval.get("type", {}).get("id") == code
        ]
        if multiple:
            return " / ".join(vals)
        if vals:
    ...
    def filter_attachments(self, src, val):
        ...
        for subval in val or []:
            ...
            if legend_attachment:
                ...
                legend = name_attachment
            result.append(
                (subval["traductionFichiers"][0]["url"], legend, copyright_attachment)
            )
        return result
...
class TouristicEventApidaeParser(AttachmentApidaeParserMixin, ApidaeParser):
    ...
    fields = {
        ...
        "duration": (
            "ouverture.periodesOuvertures.0.horaireOuverture",
    ...
    def filter_capacity(self, src, val):
        ...
        if val.isnumeric():
            ...
            self.add_warning(
                f"Field {src} can't populate capacity field value: '{val}' isn't numeric"
            )
            return None
    ...
    def filter_practical_info(self, src, val):
        ...
        if ouverture:
            ouverture = (
                "<b>Ouverture:</b><br>" + "<br>".join(ouverture.splitlines()) + "<br>"
            )
        if capacite:
        ...
        if paiement:
            paiement = (
                ...
                + ", ".join([i["libelleFr"] for i in paiement])
                + "<br>"
            )
        if services:
            services = (
                "<b>Services:</b><br>"
                + ", ".join([i["libelleFr"] for i in services])
                + "<br>"
            )
        if langues:
            langues = (
                "<b>Langues Parlées:</b><br>"
                + ", ".join([i["libelleFr"] for i in langues])
                + "<br>"
            )
        if localisation:
            localisation = (
                "<b>Accès:</b><br>" + "<br>".join(localisation.splitlines()) + "<br>"
            )
        datemodif = datetime.datetime.strptime(datemodif[:10], "%Y-%m-%d").strftime(
            "%d/%m/%Y"
        )
...
class TouristicContentApidaeParser(
    AttachmentApidaeParserMixin, TouristicContentMixin, ApidaeParser
):
...
class EspritParcParser(AttachmentParserMixin, TouristicContentMixin, Parser):
    ...
    natural_keys = {"category": "label", "type1": "label", "type2": "label"}
    ...
    def filter_contact(self, src, val):
        ...
        return "<br>".join(
            [
                part
                for part in (address, cp_com, telephone, gsm, fax, facebook, twitter)
                if part
            ]
        )
    ...
    def filter_type1(self, src, val):
        ...
        if val:
            ...
            for subval in val:
                try:
                    dst.append(
                        TouristicContentType1.objects.get(
                            category=self.obj.category, label=subval
                        )
                    )
                except TouristicContentType1.DoesNotExist:
                    self.add_warning(
                        _(
                            "Type 1 '{subval}' does not exist for category '{cat}'. Please add it"
                        ).format(subval=subval, cat=self.obj.category.label)
                    )
        return dst
    ...
    def filter_type2(self, src, val):
        ...
        if val:
            ...
            for subval in val:
                try:
                    dst.append(
                        TouristicContentType2.objects.get(
                            category=self.obj.category, label=subval
                        )
                    )
                    ...
                    self.add_warning(
                        _(
                            "Type 2 '{subval}' does not exist for category '{cat}'. Please add it"
                        ).format(subval=subval, cat=self.obj.category.label)
                    )
        return dst
...
class LEITouristicEventParser(LEIParser):
    ...
    def filter_duration(self, src, val):
        ...
        d0 = datetime.date(
            int(date_y_m_d_begin[2]), int(date_y_m_d_begin[1]), int(date_y_m_d_begin[0])
        )
        d1 = datetime.date(
            int(date_y_m_d_end[2]), int(date_y_m_d_end[1]), int(date_y_m_d_end[0])
        )
        delta = d1 - d0
...
class GeotrekTouristicContentParser(GeotrekParser):
    ...
    m2m_replace_fields = {"type1": "types", "type2": "types"}
    ...
    def __init__(self, *args, **kwargs):
        ...
        response = self.request_or_retry(
            f"{self.url}/api/v2/touristiccontent_category/",
        )
        ...
        for r in response.json()["results"]:
            for type_category in r["types"]:
                ...
                for value in values:
                    if id_category % 10 == 1:
                        self.field_options["type1"]["mapping"][value["id"]] = (
                    ...
                    if id_category % 10 == 2:
                        self.field_options["type2"]["mapping"][value["id"]] = (
                            self.replace_mapping(
                                value["label"][
                                    settings.MODELTRANSLATION_DEFAULT_LANGUAGE
                                ],
...
class GeotrekTouristicEventParser(GeotrekParser):
    ...
    categories_keys_api_v2 = {"structure": "name", "type": "type", "source": "name"}
    natural_keys = {"structure": "name", "type": "type", "source": "name"}
...
class GeotrekInformationDeskParser(GeotrekParser):
    ...
    def filter_geom(self, src, val):
        lat, lng = val
        return Point(lng, lat, srid=settings.API_SRID).transform(
            settings.SRID, clone=True
        )

    def filter_type(self, src, val):
        return self.apply_filter(
            "type", src, val["label"][settings.MODELTRANSLATION_DEFAULT_LANGUAGE]
        )
    ...
    def link_informationdesks(self, parser, datas, match_id_uuid, json_uuid_key):
        ...
        for row in datas["results"]:
            try:
                ...
                self.add_warning(
                    _(
                        "Cannot link information desk to trek: could not find "
                        "trek with UUID %(uuid)s"
                    )
                    % {"uuid": row[json_uuid_key]}
                )
            else:
                infodesks_to_set = [
                    match_id_uuid.get(val)
                    for val in row[field]
                ...
                if infodesks_to_remove:
                    current_infodesks.remove(*infodesks_to_remove)
    ...
    def end_meta(self):
        ...
        for result in datas["results"]:
            try:
                match_id_uuid[result["id"]] = InformationDesk.objects.get(
                    uuid=result["uuid"]
                )
        ...
        response = self.request_or_retry(url, params=params)
        datas = response.json()
        self.link_informationdesks(
            GeotrekTrekParser, datas, match_id_uuid, replace_fields.get("eid", "id")
        )
