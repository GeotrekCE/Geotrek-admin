...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for event in orm['tourism.TouristicEvent'].objects.exclude(public=None):
            event.target_audience = event.public.public
            event.save()

    def backwards(self, orm):
        for event in orm['tourism.TouristicEvent'].objects.exclude(target_audience=''):
            public, created = orm['tourism.TouristicEventPublic'].objects.get_or_create(public=event.target_audience)
            event.public = public
            event.save()
