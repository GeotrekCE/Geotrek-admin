...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            ...
            orm['contenttypes.contenttype'].objects.filter(app_label='trekking', model='informationdesk').update(app_label='tourism')
            orm['contenttypes.contenttype'].objects.filter(app_label='trekking', model='informationdesktype').update(app_label='tourism')

    def backwards(self, orm):
        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='tourism', model='informationdesk').update(app_label='trekking')
            orm['contenttypes.contenttype'].objects.filter(app_label='tourism', model='informationdesktype').update(app_label='trekking')
    ...
    models = {
        ...
        u'tourism.informationdesk': {
            ...
            'type': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'desks'", 'null': 'True', 'db_column': "'type'", 'to': u"orm['tourism.InformationDeskType']"}),
