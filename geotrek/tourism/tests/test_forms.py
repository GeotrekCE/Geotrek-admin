...
class TouristicEventFormTestCase(TestCase):
    def test_begin_end_time(self):
        ...
        form = TouristicEventForm(user=user, data=data)
        self.assertTrue(form.is_valid())

        # No start, end time
        form = TouristicEventForm(user=user, data={**data, **{"end_time": "10:30"}})
        ...
        form = TouristicEventForm(
            ...
            data={
                **data,
                **{"end_date": None, "start_time": "11:30", "end_time": "10:30"},
            },
        )
        self.assertFalse(form.is_valid())
        ...
        form = TouristicEventForm(
            user=user, data={**data, **{"start_time": "11:30", "end_time": "10:30"}}
        )
        self.assertFalse(form.is_valid())
        self.assertIn("Start time is after end time", str(form.errors))

        # No end time
        form = TouristicEventForm(user=user, data={**data, **{"start_time": "10:30"}})
        self.assertTrue(form.is_valid())
    ...
    def test_organizers_widget_select_multiple_with_pop(self):
        ...
        user.user_permissions.add(
            Permission.objects.get(codename="add_touristiceventorganizer")
        )
        form = TouristicEventForm(user=user, data={})
