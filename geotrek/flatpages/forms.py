...
class FlatPageForm(MoveNodeForm):
    ...
    cover_image_author = forms.CharField(
        label=_("Cover image author"), max_length=128, required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for fieldname, formfield in self.fields.items():
            if fieldname.startswith("content_"):
                self.fields[fieldname].widget = FlatPageTinyMCE(
                    attrs=self.fields[fieldname].widget.attrs
                )
...
class MenuItemForm(MoveNodeForm):
    ...
    def clean(self):
        ...
        if target_type == "link":
            link_url_loc_fieldname = build_localized_fieldname(
                "link_url", settings.MODELTRANSLATION_DEFAULT_LANGUAGE
            )
            if (
                ...
                raise ValidationError(
                    {link_url_loc_fieldname: "This field is required."}
                )
