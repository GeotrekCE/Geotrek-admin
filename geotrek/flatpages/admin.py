...
class FlatPageAdmin(BaseAdmin):
    ...
    def save_related(self, request, form, formsets, change):
        # We override `ModelAdmin.save_related` to add/update/delete the cover image attachment but
        # `ModelAdmin.save_form` may has looked like a better place. This is because Django's ModelAdmin
...
class MenuItemAdmin(BaseAdmin):
    list_display = ('label', 'portals_for_display', )
    list_filter = (
    ...
    def save_related(self, request, form, formsets, change):
        # We override `ModelAdmin.save_related` to add/update/delete the thumbnail attachment but
        # `ModelAdmin.save_form` may has looked like a better place. This is because Django's ModelAdmin
        # does not commit the object in `save_form`, it waits for formsets' validations. We override
        # `save_related` because we need the committed object (with an ID).
        # See `django.contrib.admin.options.py:L1578`
