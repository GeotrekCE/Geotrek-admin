...
class InfraFilterTestMixin():
    ...
    def test_intervention_filter(self):
        ...
        InterventionFactory(content_object=bad_topo, date=bad_date_year)

        # Good signage: intervention with the good year
        good_topo = self.factory()
        InterventionFactory(content_object=good_topo, date=good_date_year)
    ...
    def test_duplicate_implantation_year_filter(self):
        ...
        # Bad signage: intervention with wrong year
        topo_1 = self.factory()
        InterventionFactory(object_id=topo_1.pk, date=year_t)

        # Good signage: intervention with the good year
        topo_2 = self.factory()
        InterventionFactory(object_id=topo_2.pk, date=year_t)
