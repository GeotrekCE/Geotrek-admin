...
from geotrek.infrastructure.models import Infrastructure, InfrastructureTypeChoices, InfrastructureType, \
    InfrastructureMaintenanceDifficultyLevel, InfrastructureUsageDifficultyLevel, InfrastructureCondition
from geotrek.infrastructure.tests.factories import (
    InfrastructureConditionFactory,
    InfrastructureFactory,
    InfrastructureTypeFactory,
    PointInfrastructureFactory, InfrastructureMaintenanceDifficultyLevelFactory,
...
class InfrastructureReferencesTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.maintenance_difficulty_level = InfrastructureMaintenanceDifficultyLevelFactory.create()
    ...
    def test_data(self):
        ...
        r = self.client.get(
            reverse("infrastructure:infrastructure_references"), headers={"Authorization": token}
        )
        data = r.json()
        print(data)

        self.assertEqual(len(data["infrastructuretype"]), InfrastructureType.objects.all().count())
        self.assertEqual(len(data["infrastructuremaintenancedifficultylevel"]), InfrastructureMaintenanceDifficultyLevel.objects.all().count())
        self.assertEqual(len(data["infrastructureusagedifficultylevel"]), InfrastructureUsageDifficultyLevel.objects.all().count())
        self.assertEqual(len(data["infrastructurecondition"]), InfrastructureCondition.objects.all().count())
        self.assertEqual(data["infrastructuretype"][0], {"id": self.type.id, "name": self.type.label})
        self.assertEqual(data["infrastructuremaintenancedifficultylevel"][0], {"id": self.maintenance_difficulty_level.id, "name": self.maintenance_difficulty_level.label})
        self.assertEqual(data["infrastructureusagedifficultylevel"][0], {"id": self.usage_difficulty_level.id, "name": self.usage_difficulty_level.label})
        self.assertEqual(data["infrastructurecondition"][0], {"id": self.conditions.id, "name": self.conditions.label})
        self.assertEqual(data["pictogram"], {'url': 'http://testserver/static/images/infrastructure.png'})
...
class InfrastructureGTAMTest(TestCase):
    ...
    def test_data(self):
        token = self.authenticate(self.user)
        list_url = f"/api/infrastructure/drf/infrastructures?format=gtam"
        response = self.client.get(list_url, headers={"Authorization": token})
        data = response.json()

        infrastructures = [
            {'id': self.infrastructure.id,
             'date_insert': self.infrastructure.date_insert.isoformat().replace('+00:00', 'Z'),
             'date_update': self.infrastructure.date_update.isoformat().replace('+00:00', 'Z'),
             'api_geom': {
                 'type': 'LineString',
                 'coordinates': [[3.0, 46.5], [3.001304, 46.5009004]]
             },
