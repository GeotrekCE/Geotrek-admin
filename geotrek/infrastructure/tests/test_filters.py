...
class DifficultyLeversFilterTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.infra_u1_m2 = InfrastructureFactory(
            usage_difficulty=cls.usage_level_1,
            maintenance_difficulty=cls.maintenance_level_2,
        )
        cls.infra_u2_m3 = InfrastructureFactory(
            usage_difficulty=cls.usage_level_2,
            maintenance_difficulty=cls.maintenance_level_3,
        )
        cls.infra_m2 = InfrastructureFactory(
            maintenance_difficulty=cls.maintenance_level_2
        )

    def test_filter_usages(self):
        """Test usage difficulty filter on infrastructure"""
        data = {"usage_difficulty": [self.usage_level_1.pk]}
...
class InfrastructureFilterTest(TestCase):
    ...
    def test_form_should_have_signage_intervention_year_choices(self):
        ...
        choice_values = [
            choice[0] for choice in filter_set.form.fields["intervention_year"].choices
        ]
    ...
    def test_filter_by_intervention_year(self):
        filtered_infra_intervention = InfrastructureInterventionFactory(
            begin_date="2015-01-01"
        )
        non_filtered_infra_intervention = InfrastructureInterventionFactory(
