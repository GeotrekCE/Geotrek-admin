...
from geotrek.common.tests.factories import (
    AttachmentAccessibilityFactory,
    AttachmentFactory,
)
...
def delete_url_for_obj(attachment):
    return reverse(
        "common:delete_attachment_accessibility",
        kwargs={"attachment_pk": attachment.pk},
    )
...
class EntityAttachmentTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.object = TrekFactory.create(
            structure=StructureFactory(name="Another structure")
        )
    ...
    def test_list_attachments_in_details(self):
        ...
        self.assertTemplateUsed(
            response, template_name="common/attachment_accessibility_list.html"
        )

        self.assertEqual(
            1, len(AccessibilityAttachment.objects.attachments_for_object(self.object))
        )

        self.assertNotIn(b"Submit attachment", html)

        for attachment in AccessibilityAttachment.objects.attachments_for_object(
            self.object
        ):
            ...
            self.assertIn(b'<i class="bi bi-file-richtext text-primary"></i> ODT', html)
    ...
    def test_create_attachments_accessibility_object_other_structure(self):
        ...
        self.assertTemplateUsed(response, template_name="common/attachment_list.html")
        self.assertTemplateNotUsed(
            response, template_name="common/_attachment_table.html"
        )
        self.assertTemplateUsed(
            response, template_name="common/attachment_accessibility_list.html"
        )
        ...
        self.assertIn(
            b"You are not allowed to modify attachments on this object, this object is not from the same structure.",
            html,
        )
    ...
    def test_filename_generation(self):
        ...
        file.seek(0)
        attachment = AccessibilityAttachment.objects.create(
        ...
        attachment.attachment_accessibility_file = SimpleUploadedFile(
            file.name, file.read(), content_type="text/plain"
        )
...
class SetUpTestDataMixin:
    ...
    def setUpTestData(cls):
        ...
        cls.attachment = AttachmentAccessibilityFactory.create(
            content_object=cls.object
        )
        cls.object_other_structure = TrekFactory.create(
            structure=StructureFactory.create()
        )
        cls.attachment_other_structure = AttachmentAccessibilityFactory.create(
            content_object=cls.object_other_structure
        )
...
class UploadAddAttachmentTestCase(SetUpTestDataMixin, TestCase):
    ...
    def test_upload_redirects_to_trek_detail_url(self):
        ...
        self.assertEqual(
            response["location"], f"{self.object.get_detail_url()}?tab=attachments"
        )
        self.assertEqual(3, AccessibilityAttachment.objects.count())

        self.client.force_login(user=self.user)
        response = self.client.post(
            add_url_for_obj(self.object_other_structure), data=self.attachmentPostData()
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response["location"], f"{self.object_other_structure.get_detail_url()}"
        )
        self.assertEqual(3, AccessibilityAttachment.objects.count())
...
class UploadUpdateAttachmentTestCase(SetUpTestDataMixin, TestCase):
    ...
    def test_post_update_url(self):
        self.client.force_login(user=self.superuser)
        response = self.client.post(
            update_url_for_obj(self.attachment),
            data=self.attachmentPostData(),
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
...
class UploadDeleteAttachmentTestCase(SetUpTestDataMixin, TestCase):
    ...
    def test_get_delete_without_perms_url(self, mocke):
        ...
        self.assertIn(
            b"You are not allowed to delete this attachment.", response.content
        )
...
class ModelAttachmentAccessibilityTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.attachment = AttachmentAccessibilityFactory.create(
            attachment_accessibility_file=get_dummy_uploaded_image(name="bar.png"),
    ...
    def test_str(self):
        self.assertEqual(
            f"foo attached {self.attachment.attachment_accessibility_file}",
            str(self.attachment),
        )
...
class ServeAttachmentTestCase(TestCase):
    ...
    def test_get_attachment_do_not_exist(self):
        response = self.client.get(
            f"/media/attachments_accessibility/trekking_trek/{self.object.pk}/doesnotexist.png"
        )
    ...
    def test_get_attachment_on_model_without_generic(self):
        ...
        attachment = AttachmentAccessibilityFactory.create(
            attachment_accessibility_file=get_dummy_uploaded_image(name="bar.png"),
            creator=self.user,
            content_object=obj,
            title="",
    ...
    def test_attachment_is_larger_max_size(self):
        ...
        response = self.client.post(
            ...
            data={
                ...
                "title": "A title",
                "legend": "A legend",
                "attachment_accessibility_file": SimpleUploadedFile(
    ...
    def test_attachment_is_not_wide_enough(self):
        ...
        response = self.client.post(
            ...
            data={
                ...
                "legend": "A legend",
                "attachment_accessibility_file": SimpleUploadedFile(
                    small_file.name, small_file.read(), content_type="image/png"
                ),
                "author": "newauthor",
                "info_accessibility": "slope",
                "next": f"{self.object.get_detail_url()}?tab=attachments",
