...
class AttachmentAdminTest(TestCase):
    ...
    def test_changelist_attachment_filter_content_id(self):
        list_url = reverse("admin:common_attachment_changelist")
        data = {"content_type": ContentType.objects.get_for_model(POI).pk}
    ...
    def test_attachment_can_be_change(self):
        ...
        response = self.client.post(
            change_url, {"title": "Coucou", "filetype": file_type.pk, "starred": True}
        )
...
class MergeActionAdminTest(TestCase):
    ...
    def test_merge_actions_many2many(self):
        ...
        data = {
            "action": "apply_merge",
            "_selected_action": Theme.objects.all().values_list("pk", flat=True),
        }
        ...
        self.assertEqual(
            self.trek.themes.first().label, "Autre theme + Theme 1 + Theme 2"
        )
    ...
    def test_merge_actions_2_many2many(self):
        ...
        data = {
            "action": "apply_merge",
            "_selected_action": Theme.objects.filter(
                label__in=["Theme 1", "Autre theme"]
            ).values_list("pk", flat=True),
        }
        self.client.post(reverse("admin:common_theme_changelist"), data, follow=True)
        self.assertEqual(Theme.objects.count(), 2)
        self.assertEqual(
            Theme.objects.exclude(label="Theme 2").first().label,
            "Autre theme + Theme 1",
        )
    ...
    def test_merge_actions_fk(self):
        ...
        data = {
            "action": "apply_merge",
            "_selected_action": DifficultyLevel.objects.filter(
                difficulty__in=["Dif 1", "Dif 2"]
            ).values_list("pk", flat=True),
        }
        self.client.post(
            reverse("admin:trekking_difficultylevel_changelist"), data, follow=True
        )
        self.assertEqual(DifficultyLevel.objects.count(), 2)
        self.assertEqual(
            DifficultyLevel.objects.exclude(difficulty="Dif 3").first().difficulty,
            "Dif 1 + Dif 2",
        )
...
class HDViewPointAdminTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.vp = HDViewPointFactory(
            content_object=cls.trek, annotations_categories={"1": f"{cls.category.pk}"}
        )
    ...
    def test_changelist_hdviewpoint(self):
        ...
        self.assertContains(
            response,
            f'<a data-pk="{self.vp.pk}" href="{self.vp.full_url}" >{self.vp.title}</a>',
        )

    def test_nodelete_annotation_category(self):
        delete_url = reverse(
            "admin:common_annotationcategory_delete", args=(self.category.pk,)
        )
    ...
    def test_delete_annotation_category(self):
        delete_url = reverse(
            "admin:common_annotationcategory_delete", args=(self.category_2.pk,)
        )
        response = self.client.get(delete_url)
        self.assertEqual(response.status_code, 200)
