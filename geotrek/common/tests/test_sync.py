...
class SyncRandoTilesTest(VarTmpTestCase):
    ...
    def test_tiles(self, mock_tileslist, mock_tiles):
        ...
        management.call_command('sync_rando', os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync'), url='http://localhost:8000', verbosity=2,
                                languages='en', stdout=output)
        zfile = zipfile.ZipFile(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'zip', 'tiles', 'global.zip'))
        ...
        zfile_trek = zipfile.ZipFile(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'zip', 'tiles', '{}.zip'.format(trek_multi.pk)))
    ...
    def test_tiles_with_treks(self, mock_tileslist, mock_tiles, mock_prepare):
        ...
        for finfo in zfile.infolist():
            ...
            self.assertEqual(ifile.read(), b'I am a png')
        self.assertIn("zip/tiles/global.zip", output.getvalue())
        zfile_trek = zipfile.ZipFile(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'zip', 'tiles', '{pk}.zip'.format(pk=trek.pk)))
        for finfo in zfile_trek.infolist():
            ifile_trek = zfile_trek.open(finfo)
    ...
    def test_tiles_with_treks_source_portal(self, mock_tileslist, mock_tiles, mock_prepare):
        ...
        management.call_command('sync_rando', os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync'), url='http://localhost:8000',
...
class SyncRandoFailTest(VarTmpTestCase):
    def test_fail_directory_not_empty(self):
        os.makedirs(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'other'))
    ...
    def test_fail_url_ftp(self):
        with self.assertRaisesRegex(CommandError, "url parameter should start with http:// or https://"):
            management.call_command('sync_rando', os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync'), url='ftp://localhost:8000',
    ...
    def test_attachments_missing_from_disk(self, mocke):
        ...
        self.assertFalse(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'mobile', 'nolang', 'media', 'trekking_trek')))
...
class SyncTest(VarTmpTestCase):
    ...
    def test_sync_multiple_time(self):
        management.call_command('sync_rando', os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync'), url='http://localhost:8000', skip_tiles=True, languages='en',
                                skip_pdf=True, verbosity=2, stdout=StringIO())
        with open(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'en', 'treks.geojson'), 'r') as f:
            treks = json.load(f)
    ...
    def test_only_external_public_pdf(self, trek):
        ...
        self.assertFalse(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'en', 'treks', str(self.trek.pk), '%s.pdf' % self.trek.slug)))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'en', 'treks', str(trek.pk), '%s.pdf' % trek.slug)))
    ...
    def test_sync_pdf_all_languages(self, trek):
        ...
        management.call_command('sync_rando', os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync'), url='http://localhost:8000', verbosity=2,
                                skip_pdf=False, skip_tiles=True, stdout=output)
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'en', 'treks', str(trek.pk), '%s.pdf' % trek.slug)))
        self.assertFalse(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek.pk), '%s.pdf' % trek.slug)))
        self.assertFalse(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek.pk), '%s.pdf' % trek.slug)))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek.pk), 'it.pdf')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek.pk), 'fr.pdf')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek.pk), 'it.gpx')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek.pk), 'fr.gpx')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek.pk), 'it.kml')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek.pk), 'fr.kml')))

        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek_2.pk), 'it_2.pdf')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek_2.pk), 'fr_2.pdf')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek_2.pk), 'it_2.gpx')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek_2.pk), 'fr_2.gpx')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'it', 'treks', str(trek_2.pk), 'it_2.kml')))
        self.assertTrue(os.path.exists(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'fr', 'treks', str(trek_2.pk), 'fr_2.kml')))
...
class SyncComplexTest(VarTmpTestCase):
    ...
    def get_coordinates(self, geojsonfilename):
        with open(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'en', geojsonfilename), 'r') as f:
    ...
    def test_sync_with_multipolygon_sensitive_area(self):
        ...
        area.species.practices.add(SportPracticeFactory.create(name='Terrestre'))
        area.save()
        management.call_command('sync_rando', os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync'), with_signages=True, with_infrastructures=True,
        ...
        with open(os.path.join('var', 'tmp', 'sync_rando', 'tmp_sync', 'api', 'en', 'sensitiveareas.geojson'), 'r') as f:
            area = json.load(f)
            # there are 2 areas
