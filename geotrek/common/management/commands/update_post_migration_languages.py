...
from geotrek.common.models import Label, TargetPortal
        ...
        for lang in settings.MODELTRANSLATION_LANGUAGES:
            ...
            with translation.override(lang, deactivate=True):
                ...
                TargetPortal.objects.filter(
                    ...
                    **{
                        build_localized_fieldname("description", lang): _(
                            "Geotrek is a web app allowing you to prepare your "
                            "next trekking trip !"
                        )
                    }
                )
                ...
                Label.objects.filter(pk=1).filter(
                    Q(**{build_localized_fieldname("name", lang): ""})
                    | Q(**{build_localized_fieldname("name", lang): None})
                ).update(
                    **{
                        build_localized_fieldname("name", lang): _(
                            "Is in the midst of the park"
                        )
                    }
                )
                Label.objects.filter(pk=1).filter(
                    Q(**{build_localized_fieldname("advice", lang): ""})
                    | Q(**{build_localized_fieldname("advice", lang): None})
                ).update(
                    **{
                        build_localized_fieldname("advice", lang): _(
                            "The national park is an unrestricted natural area but "
