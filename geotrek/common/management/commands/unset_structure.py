...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--list", action="store_true", help="Show available models to manage"
        )
    ...
    def get_new_fk(self, old_fk, RelatedModel, related_fields):
        if old_fk and old_fk.structure:
            kwargs = {
                field.name: getattr(old_fk, field.name)
                for field in related_fields
                if field.name not in ["date_insert", "date_update"]
            }
            ...
            for Model in app.get_models():
                ...
                for field in Model._meta.get_fields():
                    ...
                    if not RelatedModel:
                        continue
                    if "structure" not in [
                        f.name for f in RelatedModel._meta.get_fields()
                    ]:
                        continue
                    if not RelatedModel._meta.get_field("structure").null:
                        continue
                    self.items[RelatedModel].append(
                        (Model, field.name, isinstance(field, ManyToManyField))
                    )
    ...
    def handle_related_model(self, RelatedModel, subitems):
        ...
        related_fields = [
            field for field in RelatedModel._meta.get_fields() if not field.auto_created
        ]
        ...
        for Model, fk_name, m2m in subitems:
            ...
            for obj in Model.objects.all():
                ...
                if m2m:
                    new_fk = [
                        self.get_new_fk(value, RelatedModel, related_fields)
                        for value in old_fk.all()
                    ]
                    old_fk.set(new_fk)
                else:
        ...
        if related_objs and self.options["verbosity"] > 0:
            self.stdout.write(
                "  Delete {}".format(", ".join([str(obj) for obj in related_objs]))
            )
        related_objs.delete()
