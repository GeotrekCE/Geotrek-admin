...
class AttachmentAdmin(admin.ModelAdmin):
    ...
    @admin.display(description=_("Linked content"))
    def content_link(self, obj):
        """Returns content object link"""
        try:
            assert hasattr(obj.content_object, "_entity"), (
                f"Unregistered model {obj.content_type}"
            )
...
class AnnotationCategoryAdmin(MergeActionMixin, TabbedTranslationAdmin):
    ...
    def has_delete_permission(self, request, obj=None):
        if obj:
            ...
            annotations_categories = common_models.HDViewPoint.objects.values_list(
                "annotations_categories", flat=True
            )
            used_annotations_categories = set(
                val for dic in annotations_categories for val in dic.values()
            )
            return (
                request.user.has_perm("common.delete_annotationcategory")
                and str(obj.pk) not in used_annotations_categories
            )
...
class HDViewPointAdmin(admin.ModelAdmin):
    ...
    def update_link(self, obj):
        ...
        return format_html(
            '<a data-pk="{}" href="{}" >{}</a>', obj.pk, obj.full_url, obj.title
        )

    @admin.display(description=_("Related to"))
